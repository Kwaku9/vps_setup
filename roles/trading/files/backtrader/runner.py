    ...
    def notify_trade(self, trade):
        if trade.isopen:
            ...
            open_trade.update({
                ...
                "pnl_pct": round(trade.pnl / (trade.price * abs(trade.size)) * 100, 4) if trade.price else 0,
...
def fetch_ohlc_from_vm(symbol, timeframe, start, end):
    """Fetch OHLC data from VictoriaMetrics."""
    ...
    step_map = {"1m": "60s", "5m": "300s", "15m": "900s", "1h": "3600s", "4h": "14400s", "1d": "86400s", "1w": "604800s"}
    step = step_map.get(timeframe, "86400s")

    metrics = {}
    for field in ["open", "high", "low", "close", "volume"]:
        url = f"{VM_URL}/api/v1/query_range"
        params = {
            "query": f'ohlc_{field}{{symbol="{symbol}",timeframe="{timeframe}"}}',
            "start": start_ts,
            "end": end_ts,
            "step": step,
        }
        ...
        if data["data"]["result"]:
            ...
            metrics[field] = {int(v[0]): float(v[1]) for v in values}
...
def main():
    ...
    df = fetch_ohlc_from_vm(args.symbol, base_tf, args.start, args.end)
