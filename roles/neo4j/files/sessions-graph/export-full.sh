...
q sg_sessions.csv \
  "SELECT session_uuid, id AS pg_id, project_id, translate(left(coalesce(title,''),400), E'\n\r\t' || chr(34), '    ') AS title, coalesce(source,'') AS source, coalesce(model,'') AS model, coalesce(status,'') AS status, coalesce(session_category,'') AS category, coalesce(started_at::text,'') AS started_at, coalesce(ended_at::text,'') AS ended_at, coalesce(duration_seconds,0) AS duration_seconds, coalesce(total_messages,0) AS total_messages, coalesce(total_turns,0) AS total_turns, coalesce(total_tool_calls,0) AS total_tool_calls, coalesce(git_branch,'') AS git_branch, coalesce(cli_version,'') AS cli_version FROM sessions.sessions WHERE session_uuid IS NOT NULL"
